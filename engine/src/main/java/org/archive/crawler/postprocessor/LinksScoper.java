...
/**
 * Determine which extracted links are within scope. TODO: To test scope,
 * requires that Link be converted to a CrawlURI. Make it so don't have to make
 * a CrawlURI to test if Link is in scope.
 * <p>
 * Since this scoper has to create CrawlURIs, no sense discarding them since
 * later in the processing chain CrawlURIs rather than Links are whats needed
 * scheduling extracted links w/ the Frontier (Frontier#schedule expects
 * CrawlURI, not Link). This class replaces Links w/ the CrawlURI that wraps the
 * Link in the CrawlURI.
 * 
 * @deprecated Use CandidatesProcessor and CandidateChain/CandidateScoper
 *             instead
 * 
 * @author gojomo
 * @author stack
...
public class LinksScoper extends Scoper {
    ...
    private static Logger LOGGER = Logger
            .getLogger(LinksScoper.class.getName());
    ...
    /**
     * DecideRules applied after an URI has been rejected. If the rules accept,
     * the URI is logged (if the logging level is INFO). Depends on
     * {@link Scoper#OVERRIDE_LOGGER} being enabled.
     */
    {
        setLogRejectsRule(new RejectDecideRule());
    }

    public AcceptRule getLogRejectsRule() {
        return (AcceptRule) kp.get("logRejectsRule");
    }

    public void setLogRejectsRule(AcceptRule rule) {
    ...
    protected void innerProcess(final CrawlURI puri) {
        ...
        for (Link wref : curi.getOutLinks())
            try {
                int directive = getSchedulingFor(curi, wref,
                        preferenceDepthHops);
                CrawlURI caURI = curi.createCrawlURI(curi.getBaseURI(), wref,
                        directive,
                ...
                loggerModule.logUriError(e, curi.getUURI(), wref
    ...
    /**
     * The CrawlURI has a prerequisite; apply scoping and update Link to
     * CrawlURI in manner analogous to outlink handling.
     * 
     * @param curi
    ...
    // TODO: move similar outOfScope logging to
    // CandidatesProcessor/CandidateChain/CandidateScoper
    protected void outOfScope(CrawlURI caUri) {
        super.outOfScope(caUri);
        if (!LOGGER.isLoggable(Level.INFO)) {
            return;
        }
        if (getLogRejectsRule().accepts(caUri)) {
    ...
    /**
     * Determine scheduling for the <code>curi</code>. As with the LinksScoper
     * in general, this only handles extracted links, seeds do not pass through
     * here, but are given MEDIUM priority. Imports into the frontier similarly
     * do not pass through here, but are given NORMAL priority.
     */
